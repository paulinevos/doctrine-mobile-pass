...
The Apple and Google pairs aren't interchangeable. Each platform has its own namespace (`Vos\DoctrineMobilePass\Builders\Apple\...` and `Vos\DoctrineMobilePass\Builders\Google\...`), so if you want to support both platforms for the same conceptual pass, you build twice.
